...
class Moderation(commands.Cog):
    ...
    async def _purge(
        ...
        await ctx.send(
            can_delete=True,
            embed=discord.Embed(
                description=f"Cleared `{len(msgs)}` messages.",
                color=discord.Color.dark_red(),
            ),
        )

    @commands.command(
        name="ban", description="Ban a member.", usage="ban <member> [reason]"
    )
    ...
    async def _ban(
        ...
        *,
        ...
        await ctx.reply(
            embed=discord.Embed(
                description=f"Banned {member.mention} for `{reason}`. (by {ctx.author.mention})",
                color=discord.Color.dark_red(),
            )
        )

    @commands.command(
        name="kick", description="Kick a member.", usage="kick <member> [reason]"
    )
    ...
    async def _kick(
        ...
        *,
        ...
        await ctx.reply(
            embed=discord.Embed(
                description=f"Kicked {member.mention} for `{reason}`. (by {ctx.author.mention})",
                color=discord.Color.dark_red(),
            )
        )
    ...
    @commands.command(
        name="slowmode",
        description="Edit slowmode of channel.",
    ...
    async def _slowmode(
        ...
        if seconds < 0 or seconds > 21600:
            raise commands.BadArgument("Slowmode must been between 0 and 21,600.")
    ...
    async def _unban(self, ctx: ApolloContext, user: commands.UserConverter) -> None:
        ...
        await ctx.reply(
            embed=discord.Embed(
                description=f"Unbanned {user.mention} (by {ctx.author.mention}).",
                color=discord.Color.dark_red(),
            )
        )
    ...
    @commands.command(
        name="setnick",
        description="Set nick of member. Set to 'reset' to reset.",
        usage="nick <member> <nick>",
    )
    @commands.has_guild_permissions(manage_nicknames=True)
    async def _setnick(
        self, ctx: ApolloContext, member: commands.MemberConverter, nick: str
    ):
        check_hierarchy(ctx.author, member)
        if nick == "reset":
        ...
        await ctx.reply(
            embed=discord.Embed(
                description=f"Changed {member.mention} nickname to `{nick}`.",
                color=discord.Color.dark_red(),
            )
        )
