...
class Utility(commands.Cog):
    ...
    @commands.command(
        name="pypi",
        description="Shows details of a python package.",
        usage="pypi <package>",
    )
    ...
    async def _pypi(self, ctx: ApolloContext, package: str) -> None:
        ...
        embed = discord.Embed(
            ...
            url=data.get("project_url", "https://pypi.org/"),
            color=0x2F3136,
        )
        if data["version"] != "":
            embed.add_field(name="Version", value=data.get("version", "N/A"))
        if data["license"] != "":
            embed.add_field(name="License", value=data.get("license", "N/A"))
        if data["author"] != "":
        ...
        if data.get("project_urls", None) is not None:
            for title, url in data.get("project_urls", {}).items():
                embed.add_field(name=title, value=f"[click]({url})", inline=True)
    ...
    @commands.command(
        name="npm",
        description="Shows details of a node package.",
        usage="npm <package>",
    )
    @commands.cooldown(1, 2, commands.BucketType.user)
    async def _npm(self, ctx: ApolloContext, package: str):
        async with ctx.typing():
            data = await (
                await self.bot.session.get(f"https://api.npms.io/v2/package/{package}")
            ).json()
        ...
        for title, url in data["links"].items():
            if title != "npm":
                embed.add_field(name=title.capitalize(), value=f"[click]({url})")
    ...
    @commands.command(
        name="deno",
        description="Shows details of a deno package.",
        usage="deno <package>",
    )
    @commands.cooldown(1, 2, commands.BucketType.user)
    async def _deno(self, ctx: ApolloContext, package: str) -> None:
        async with ctx.typing():
            data = await (
                await self.bot.session.get(f"https://api.deno.land/modules/{package}")
            ).json()
            if data["success"]:
                data = data["data"]
                version = await (
                    await self.bot.session.get(
                        f"https://cdn.deno.land/{data['name']}/meta/versions.json"
                    )
                ).json()
                embed = discord.Embed(
                    title=data["name"],
    ...
    @commands.command(
        name="github",
        description="Shows details of a deno repository.",
    ...
    async def _github(self, ctx: ApolloContext, repository: str) -> None:
        async with ctx.typing():
            data = await (
                await self.bot.session.get(f"https://api.github.com/repos/{repository}")
            ).json()
        if "message" not in data:
            embed = discord.Embed(
                title=data["full_name"], url=data["html_url"], color=0x2F3136
            )
            embed.set_image(
    ...
    @commands.command(
        name="execute", description="Run code.", usage="execute <language> <code>"
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def _execute(
        self, ctx: ApolloContext, language: str, *, code: codeblock_converter
    ) -> None:
        async with ctx.typing():
