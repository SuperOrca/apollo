...
class Meta(commands.Cog):
    ...
    async def _status(self) -> None:
        await self.bot.change_presence(
            activity=discord.Game(f"@Apollo help | {len(self.bot.guilds)} guilds")
        )
    ...
    async def _uptime(self, ctx: ApolloContext) -> None:
        await ctx.reply(
            embed=discord.Embed(
                description=f"The bot has been online for `{self.get_uptime()}`.",
                color=discord.Color.blurple(),
            )
        )
    ...
    async def _info(self, ctx: ApolloContext) -> None:
        ...
        embed.add_field(name="Version", value=self.bot.__version__, inline=True)
    ...
    @commands.command(
        name="source",
        description="Shows source of the bot.",
        aliases=["src", "contribute", "contrib"],
    )
    ...
    @commands.command(
        name="prefix", description="Change the bot prefix.", usage="prefix [prefix]"
    )
    async def _prefix(self, ctx: ApolloContext, prefix: PrefixConverter = None) -> None:
        if ctx.author.guild_permissions.administrator and prefix:
            await self.bot.db.execute(
                "INSERT OR REPLACE INTO prefixes VALUES (:id, :prefix)",
                values={"id": ctx.guild.id, "prefix": prefix},
            )
            await ctx.reply(
                embed=discord.Embed(
                    description=f"Set the server prefix to `{prefix}`.",
                    color=discord.Color.blurple(),
                )
            )
        else:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    description=f"The current server prefix is `{prefix}`.",
                    color=discord.Color.blurple(),
                )
            )
