...
class TypeRacer:
	...
	async def wait_for_tr_response(self, ctx: commands.Context, text: str, *, timeout: int):
		...
		def format_line(
			i, x): return f" • {emoji_map[i]} | {x['user'].mention} in {x['time']:.2f}s | **WPM:** {x['wpm']:.2f} | **ACC:** {x['acc']:.2f}%"
		...
		while True:
			...
			self._embed.description += format_line(
				len(winners), winners[len(winners)-1]) + "\n"
	...
	async def start(
		...
		embed_color: Optional[Union[discord.Color, int]
		...
		if mode == "sentence":
			...
			text = " ".join([random.choice(self.GRAMMAR_WORDS).lower()
							for _ in range(15)])
		else:
			raise TypeError(
